...
function ManagePackages() {
  ...
  const fetchPackages = async () => {
    try {
      ...
      // Reference to packages in Firebase
      ...
      packagesSnapshot.forEach((childSnapshot) => {
        ...
        // If admin, get all packages, if vendor, only get their packages
        if (userRole === 'admin' || (userRole === 'vendor' && packageData.vendorId === user.uid)) {
          packagesData.push({
  ...
  const filteredPackages = packages.filter(pkg => {
    ...
    const matchesFilter = currentFilter === 'all' ? true :
                         currentFilter === 'active' ? pkg.status === 'approved' && pkg.available :
                         currentFilter === 'inactive' ? pkg.status === 'approved' && !pkg.available :
                         currentFilter === 'pending' ? pkg.status === 'pending' :
                         currentFilter === 'rejected' ? pkg.status === 'rejected' : true;
        ...
        {filteredPackages.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {filteredPackages.map((pkg) => (
              <div key={pkg._id} className="bg-white overflow-hidden shadow rounded-lg">
                <div className="relative h-48 w-full overflow-hidden">
                  <img
                    className="h-full w-full object-cover"
                    src={pkg.images && pkg.images.length > 0 
                      ? `http://localhost:5000/uploads/${pkg.images[0]}`
                ...
                <div className="p-6">
                  ...
                  {deleteId === pkg._id ? (
                    ...
                    <div className="flex space-x-2">
                      ...
                      {pkg.status === 'approved' && (
                        <button
                          onClick={() => handleToggleStatus(pkg._id, pkg.available)}
                          disabled={actionLoading}
                      ...
                      {/* Delete button is always visible for admins */}
                      <button
